...
- `[ ]` **PAR-22** `(新)` `namespace` 声明与限定名 `ns::name` 语法（现缺失；`src/libsafec` 已使用 `namespace safec`）。
...
- `[ ]` **MOD-15** `(新)` **名称修饰方案（规范性）**：C ABI、方法、模板单态化、`static` 成员、namespace 的完整规则（细化 MOD-08）。
...
- `[ ]` **TYP-22** `(新)` **整数提升与常规算术转换**：转换等级、signed×unsigned 混合规则（细化 TYP-19）。
- `[ ]` **TYP-23** `(新)` **隐式/显式转换矩阵**：标量、指针、数组、struct/class、enum、Optional/Result 的完整转换表。
- `[ ]` **TYP-24** `(新)` **空指针常量语义**：`nullptr`/`NULL`/`0` 与指针/bool 的转换规则。
- `[ ]` **TYP-25** `(新)` enum 默认底层类型、枚举常量作用域与限定访问（细化 TYP-09）。
- `[ ]` **TYP-26** `(新)` 字符串字面量类型、`str`/`String` 生命周期与所有权（细化 FMT-01/02）。
- `[ ]` **TYP-27** `(新)` 聚合类型 ABI：按值传参/返回（byval/sret）、端序、默认对齐与成员内边距（细化 MEM-09）。
...
- `[ ]` **SEM-15** `(新)` **求值顺序与序列点**：表达式/函数实参求值顺序，对齐 C 标准（细化 SEM-13）。
- `[ ]` **SEM-16** `(新)` **重载解析规则**：可行候选、最佳匹配、歧义诊断、模板实例参与（细化 FUN-07/GEN-06）。
- `[ ]` **SEM-17** `(新)` **可重载运算符集合与签名约定**：允许重载的运算符、参数/返回约束。
- `[ ]` **SEM-18** `(新)` **`defer` 语义**：注册顺序、作用域退出执行顺序、与 `return`/`break`/`continue` 的交互（细化 FUN-10）。
...
- `[ ]` **FUN-14** `(新)` `to_string`/`to_hash`/`equals` 扩展点的精确签名与查找规则（自由函数/成员方法两条路径）。
...
- `[ ]` **MEM-14** `(新)` 聚合传参/返回 ABI（byval/sret）与栈布局（配合 TYP-27）。
- `[ ]` **MEM-15** `(新)` 端序与默认对齐规则；`[[packed]]`/`[[align]]` 交互（细化 MEM-09/DEC-08）。
- `[ ]` **MEM-16** `(新)` 调用约定：C 默认约定细节、可变参数约定、参数寄存器/栈规则（细化 MEM-11）。
...
- `[ ]` **STD-26** `(新)` 扩展点库约定：`to_string`/`to_hash`/`equals` 标准签名与示例（落实 STD-09/FUN-14）。
...
- `[ ]` **P0-08** `(新)` **namespace 支持**（PAR-22、MOD-12、DEC-17）——现有 `libsafec` 已依赖，属刚需。
